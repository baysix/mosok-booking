...
import { getMembers } from '@/services/membership.service';
...
function ProductsTab() {
  ...
  const fetchProducts = useCallback(async () => {
    try {
      const data = await getMyPrayerProducts();
      setProducts(data);
    } catch {
      // silent
    } finally {
  ...
  const handleToggleActive = async (product: PrayerProduct) => {
    try {
      await updatePrayerProduct(product.id, { isActive: !product.isActive });
      await fetchProducts();
  ...
  const handleDelete = async (product: PrayerProduct) => {
    ...
    try {
      await deletePrayerProduct(product.id);
      await fetchProducts();
    ...
    <div>
      {products.length === 0 ? (
        <div className="text-center py-16">
          ...
          <p className="text-sm text-gray-400 mb-4">등록된 기원 상품이 없습니다</p>
          <button
...
function ProductFormModal({
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (isEdit && product) {
        await updatePrayerProduct(product.id, {
...
function OrdersTab() {
  const [orders, setOrders] = useState<PrayerOrderWithUser[]>([]);
  const [loading, setLoading] = useState(true);
  ...
  const fetchOrders = useCallback(async () => {
    setLoading(true);
    try {
      const data = await getMasterPrayerOrders(statusFilter || undefined);
      setOrders(data);
    } catch {
      // silent
    } finally {
  ...
  useEffect(() => {
    getMyPrayerProducts().then(setProducts).catch(() => {});
  }, []);

  const handleStatusChange = async (id: string, status: PrayerOrderStatus) => {
    setUpdatingId(id);
    try {
      await updateMasterPrayerOrderStatus(id, status);
      await fetchOrders();
    } catch {
      // silent
    } finally {
    ...
    <div>
      ...
      {loading ? (
        ...
        <div className="text-center py-12">
          ...
          <p className="text-sm text-gray-400">기원 내역이 없습니다</p>
        </div>
      ) : (
        <div className="space-y-2">
          {orders.map((order) => {
      ...
      {modalOpen && (
        <ManualPrayerModal
          ...
          onSubmit={async (data) => {
            await createManualPrayerOrder(data);
            await fetchOrders();
          }}
        />
      )}
...
function ManualPrayerModal({
  ...
  onClose: () => void;
  onSubmit: (data: Parameters<typeof createManualPrayerOrder>[0]) => Promise<void>;
}) {
        ...
        <div className="flex-shrink-0">
          ...
          <div className="flex items-center justify-between p-4 pb-2">
            <div className="flex items-center gap-2">
              ...
              <h3 className="text-base font-bold text-gray-900">수동 기원 등록</h3>
            </div>
            <button onClick={onClose} className="p-2 rounded-lg hover:bg-gray-100 transition-colors">
              <X className="w-5 h-5 text-gray-400" />
            </button>
